...
fig_dir <- 'figures/SEM_analysis/model_summaries/mammal_gd/'
...
full_sem_PR <- psem(
  ...
  lm(mammal_gd ~ latitude, data = full_sem_data_PR),
  lm(log_resource_rent ~ latitude, data = full_sem_data_PR),
  lm(prehistory_states ~ mammal_gd, data = full_sem_data_PR),
  
  # colonisation predicted by socio-economic history and biodiversity
  glm(colonisation ~ prehistory_states + maritime_distance + land_distance + log_resource_rent + mammal_gd, family = 'binomial', data = full_sem_data_PR),
  ...
  glm(poverty_rate ~ debt + governance_strength + landlocked + agriculture + log_resource_rent + mammal_gd  + colonisation, 
...
full_sem_withLat_PR <- update(full_sem_PR, prehistory_states ~ mammal_gd + latitude, colonisation ~ prehistory_states + maritime_distance + land_distance + log_resource_rent + mammal_gd + latitude)
...
full_sem_NPR <- psem(
  ...
  # richness predicted by latitude
  lm(mammal_gd ~ latitude, data = full_sem_data_NPR),
  lm(log_resource_rent ~ latitude, data = full_sem_data_NPR),
  lm(prehistory_states ~ mammal_gd, data = full_sem_data_NPR),
  
  # colonisation predicted by socio-economic history and biodiversity
  glm(colonisation ~ prehistory_states + maritime_distance + land_distance + log_resource_rent + mammal_gd, family = 'binomial', data = full_sem_data_NPR),
  ...
  # proverty predicted by debt, geography, agriculture, and biodiversity
  glm(national_poverty_headcount ~ debt + governance_strength + landlocked + agriculture + log_resource_rent + mammal_gd  + colonisation, 
...
full_sem_MDP <- psem(
  ...
  # richness predicted by latitude
  lm(mammal_gd ~ latitude, data = full_sem_data_MDP),
  lm(log_resource_rent ~ latitude, data = full_sem_data_MDP),
  lm(prehistory_states ~ mammal_gd, data = full_sem_data_MDP),
  
  # colonisation predicted by socio-economic history and biodiversity
  glm(colonisation ~ prehistory_states + maritime_distance + land_distance + log_resource_rent + mammal_gd, family = 'binomial', data = full_sem_data_MDP),
  ...
  # proverty predicted by debt, geography, agriculture, and biodiversity
  glm(MD_poverty ~ debt + governance_strength + landlocked + agriculture + log_resource_rent + mammal_gd  + colonisation, 
...
full_sem_GINI <- psem(
  ...
  # richness predicted by latitude
  lm(mammal_gd ~ latitude, data = full_sem_data_GINI),
  lm(log_resource_rent ~ latitude, data = full_sem_data_GINI),
  lm(prehistory_states ~ mammal_gd, data = full_sem_data_GINI),
  
  # colonisation predicted by socio-economic history and biodiversity
  glm(colonisation ~ prehistory_states + maritime_distance + land_distance + log_resource_rent + mammal_gd, family = 'binomial', data = full_sem_data_GINI),
  ...
  # proverty predicted by debt, geography, agriculture, and biodiversity
  glm(gini ~ debt + governance_strength + landlocked + agriculture + log_resource_rent + mammal_gd + colonisation, 
...
model_fit_list <- list.files('figures/SEM_analysis/model_runs/mammal_gd', full.names = T)
