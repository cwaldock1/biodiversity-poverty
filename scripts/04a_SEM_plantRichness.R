...
sum_dir <- 'figures/SEM_analysis/model_summaries/plant_richness/'
...
full_sem_PR <- psem(
  ...
  plant_richness        = lm(plant_richness ~ latitude, data = full_sem_data_PR),
  minerals              = lm(minerals ~ latitude, data = full_sem_data_PR),
  prehistory_states     = glm(prehistory_states ~ plant_richness , data = full_sem_data_PR, family = 'binomial'),
  ...
  colonisation          = glm(colonisation ~ prehistory_states + maritime_distance + land_distance + 
                                minerals + plant_richness, family = 'binomial', data = full_sem_data_PR),
  ...
  poverty_rate          = glm(poverty_rate ~ debt + governance_strength + landlocked + agriculture +
                                log_resource_rent + plant_richness + colonisation, 
...
full_sem_NPR <- psem(
  ...
  # richness predicted by latitude
  plant_richness        = lm(plant_richness ~ latitude, data = full_sem_data_NPR),
  minerals              = lm(minerals ~ latitude, data = full_sem_data_NPR),
  prehistory_states     = glm(prehistory_states ~ plant_richness , data = full_sem_data_NPR, family = 'binomial'),
  
  # colonisation predicted by socio-economic history and biodiversity
  colonisation          = glm(colonisation ~ prehistory_states + maritime_distance + land_distance + 
                                minerals + plant_richness, family = 'binomial', data = full_sem_data_NPR),
...
full_sem_MDP <- psem(
  ...
  # richness predicted by latitude
  plant_richness        = lm(plant_richness ~ latitude, data = full_sem_data_MDP),
  minerals              = lm(minerals ~ latitude, data = full_sem_data_MDP),
  prehistory_states     = glm(prehistory_states ~ plant_richness , data = full_sem_data_MDP, family = 'binomial'),
  
  # colonisation predicted by socio-economic history and biodiversity
  colonisation          = glm(colonisation ~ prehistory_states + maritime_distance + land_distance + 
                                minerals + plant_richness, family = 'binomial', data = full_sem_data_MDP),
...
full_sem_GINI <- psem(
  ...
  # richness predicted by latitude
  plant_richness        = lm(plant_richness ~ latitude, data = full_sem_data_GINI),
  minerals              = lm(minerals ~ latitude, data = full_sem_data_GINI),
  prehistory_states     = glm(prehistory_states ~ plant_richness , data = full_sem_data_GINI, family = 'binomial'),
  
  # colonisation predicted by socio-economic history and biodiversity
  colonisation          = glm(colonisation ~ prehistory_states + maritime_distance + land_distance + 
                                minerals + plant_richness, family = 'binomial', data = full_sem_data_GINI),
...
# range richness
model_fit_list <- list.files('figures/SEM_analysis/model_runs/plant_richness', full.names = T, recursive = T)
