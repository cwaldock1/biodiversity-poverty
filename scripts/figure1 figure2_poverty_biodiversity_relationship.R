# Script to produce set of insets in figure 1 and figure 2
...
# Identify countries based on area of polygons in spatial dataframe (to improve visibility on maps)
...
# Map species range richness - inset for figure 1
...
# Map poverty headcount - inset for figure 1
...
biopov_labels <- cpd_biplot %>% 
  ungroup() %>% 
  # group by metrics for biodiversity and poverty
  group_by(bio_metric, pov_metric) %>% 
  nest() %>% 
  # calculate ranked correlation and linear relationship between values
  ...
  # simplify to two significant figures
  mutate(estimate = signif(estimate, 2)) %>% 
  group_by(bio_metric, pov_metric) %>% 
  # identify positions for plotting
...
# Biplots of poverty and biodiversity for Figure 1
...
# create aggregated data that are rescaled 
...
new <- cpb %>% 
  ...
  mutate(pov_minus_bio = mean_pov - mean_bio, 
         ...
         # calculate residuals for plot colour
